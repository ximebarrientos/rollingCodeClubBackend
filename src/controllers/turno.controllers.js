...
export const crearTurno = async (req, res) => {
  try {
    ...
    if (fechaSolicitada < fechaActual) {
      return res
        .status(400)
        .json({ mensaje: "No se pueden reservar turnos para fechas pasadas" });
    }
    ...
    res
      .status(201)
      .json({ mensaje: "Turno reservado con éxito", turno: turnoNuevo });
  } catch (error) {
    console.error(error);
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          mensaje:
            "Ya existe un turno reservado para esta cancha en la fecha y horario especificados",
        });
    }
...
export const obtenerTurnos = async (req, res) => {
  try {
    const listaTurnos = await Turno.find().populate(
      "canchaId",
      "nombreCancha categoriaCancha"
    );
...
export const obtenerTurnoPorId = async (req, res) => {
  try {
    const turno = await Turno.findById(req.params.id).populate(
      "canchaId",
      "nombreCancha categoriaCancha"
    );
    if (!turno) {
...
export const borrarTurnoPorID = async (req, res) => {
  try {
    ...
    if (!turnoEliminado) {
      return res
        .status(404)
        .json({ mensaje: "Turno no encontrado para eliminar" });
    }
